...
struct CubeGeneratorCircuit<F: Field> {
    pub x: Option<F>,
}
...
impl<F: Field> ConstraintSynthesizer<F> for CubeGeneratorCircuit<F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
...
#[test]
fn test_groth16_cube_generator() {
    let rng = &mut OsRng;
